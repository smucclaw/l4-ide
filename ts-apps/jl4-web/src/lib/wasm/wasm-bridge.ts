...
export class L4WasmBridge {
  ...
  private createWasiImports(): Record<string, WebAssembly.ImportValue> {
    ...
    const environ_sizes_get = (environc: number, environBufSize: number): number => {
      const memory = getMemory()
      if (memory) {
        const view = new DataView(memory.buffer)
   ...
   * Get hover information at a position
   */
  async hover(source: string, line: number, character: number): Promise<Hover | null> {
    if (!this.exports) {
      throw new Error('WASM not initialized')
    }
   ...
   */
  async completions(source: string, line: number, character: number): Promise<CompletionItem[]> {
    if (!this.exports) {
      throw new Error('WASM not initialized')
    }
