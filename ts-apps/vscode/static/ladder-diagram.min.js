var LadderDiagram = (() => {
  ...
  // node_modules/ladder-diagram/js/ladder.js
  var ladder_exports = {}
  __export(ladder_exports, {
    AllQuantifier: () => AllQuantifier,
    AnyQuantifier: () => AnyQuantifier,
    BoolVar: () => BoolVar,
    LadderDiagram: () => LadderDiagram,
  })

  // node_modules/ladder-diagram/js/Data.js
  var BoolVar = class {
    /**
  ...
  // node_modules/ladder-diagram/js/LadderDiagram.js
  var LadderDiagram = class _LadderDiagram {
    /**
     * Constructor
    ...
    constructor(circuit, box_style = 'Corners', _subgraph = false) {
      if (circuit.type == 'BoolVar') {
        circuit = new AnyQuantifier([circuit])
      }
    ...
    _create_text_cell(x, y, circuit) {
      ...
      switch (this.box_style) {
        ...
        default:
          console.error(
            `LadderDiagram._create_text_cell: Invalid this.box_styleExpected: "Corners" | "Sides". Got ${this.box_style}`
          )
      }
      for (const i of tags) {
      ...
      internal.classList.add(
        `ladder-diagram-box-${circuit.isnegated ? 'isnegated' : 'isnotnegated'}`
      )
      let truthval = null
      if (circuit.known) {
      ...
      switch (truthval) {
        case 'T':
          internal.classList.add('ladder-diagram-box-bool-T')
          break
        case 'F':
          internal.classList.add('ladder-diagram-box-bool-F')
          break
        case 'U':
        case null:
          internal.classList.add('ladder-diagram-box-bool-U')
          break
        default:
          console.error(
            `LadderDiagram._create_text_cell: Invalid bool tag in BoolVarExpected: null | 'U' | 'T' | 'F'. Got ${truthval}`
          )
      }
      return internal
    }
    _create_subgraph_cell(x, y, subcircuit) {
      ...
      let ld = new _LadderDiagram(subcircuit, this.box_style, true)
      ld.attach(internal)
      return [internal, ld]
    }
    _init_grid() {
      switch (this.graph_type) {
        case 'AllQuantifier':
          ...
          this.circuit.children.forEach((item, idx) => {
            ...
            if (item.type == 'BoolVar') {
              ;[dom_node, diagram_object] = [
                this._create_text_cell(1 + 2 * idx, 0, item),
                null,
              ]
            } else {
              ;[dom_node, diagram_object] = this._create_subgraph_cell(
                1 + 2 * idx,
                0,
                item
              )
        ...
        case 'AnyQuantifier':
          ...
          this.circuit.children.forEach((item, idx) => {
            ...
            if (item.type == 'BoolVar') {
              ;[dom_node, diagram_object] = [
                this._create_text_cell(1, idx, item),
                null,
              ]
            } else {
              ;[dom_node, diagram_object] = this._create_subgraph_cell(
        ...
        default:
          console.error(
            `LadderDiagram._init_grid: Invalid circuit type. Expected AllQuantifier | AnyQuantifier. Got ${this.graph_type}`
          )
      }
    }
    _init_drawing_ctx() {
    ...
    _draw_line(pos1, pos2, color, linewidth) {
      ...
      this.ctx.bezierCurveTo(
        0.5 * pos1[0] + 0.5 * pos2[0],
        pos1[1],
        0.5 * pos2[0] + 0.5 * pos1[0],
        pos2[1],
        pos2[0],
        pos2[1]
      )
    ...
    _dom_get_center(dom) {
      ...
      let bb = dom.getBoundingClientRect()
      return [
        bb.x + bb.width / 2 - diagbb.left,
        bb.y + bb.height / 2 - diagbb.top,
      ]
    }
    _draw_lines_allquantifier(color, linewidth) {
      this.dom_nodes.forEach((item, idx) => {
        ...
        let prev_hidden =
          idx == 0 ? this.start_point : this.dom_nodes[idx - 1][2]
    ...
    _init_lines(color = 'rgb(120,120,120)', linewidth = 0.05 * this.em_size) {
      switch (this.graph_type) {
        ...
        default:
          console.error(
            `LadderDiagram._init_lines: Invalid circuit type. Expected AllQuantifier | AnyQuantifier. Got ${this.graph_type}`
          )
      }
      if (!this.is_subgraph) {
    ...
    _draw_truth_path(
      ...
      switch (this.graph_type) {
        ...
        default:
          console.error(
            `LadderDiagram._draw_truth_path: Invalid circuit type. Expected AllQuantifier | AnyQuantifier. Got ${this.graph_type}`
          )
      }
    }
    _node_is_true(cidx) {
    ...
    _init_events() {
      ...
      this.dom_diagram.addEventListener('scroll', (event) =>
        this.redraw_lines()
      )
    ...
    get_truth_path() {
      ...
      switch (this.graph_type) {
        ...
        default:
          console.error(
            `LadderDiagram.has_truth_path: Invalid circuit type. Expected AllQuantifier | AnyQuantifier. Got ${this.graph_type}`
          )
      }
      return ret
    }
    ...
    detach() {
      if (!this.attached) {
        console.error(`LadderDiagram.detach: Diagram not attached anything`)
        return
      }
      this.attached = false
    ...
    attach(dom_parent) {
      if (this.attached) {
        console.error(`LadderDiagram.attach: Diagram already attached`)
        return
      }
      this.attached = true
      this.dom_parent = dom_parent
      this.dom_diagram = document.createElement('div')
      this.dom_diagram.classList.add(
        this.is_subgraph ? 'ladder-diagram-subgraph' : 'ladder-diagram'
      )
      ...
      if (this.circuit.header) {
        ...
        this.dom_header.appendChild(
          document.createTextNode(this.circuit.header)
        )
  ...
  return __toCommonJS(ladder_exports)
})()
