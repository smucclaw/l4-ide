...
- `doc/dev/specs/todo/BOOLEAN-MINIMIZATION-SPEC.md`
- `doc/dev/specs/todo/PARTIAL-EVAL-VISUALIZER-SPEC.md`
...
| Stable atom IDs                               |           ✅ | `atomId` is UUIDv5 derived from function + atom label + input refs; surfaced in query-plan + ladder payloads.           |
...
| --: | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---: | ----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1 | Symbolic evaluation (core)                      | `doc/dev/specs/todo/BOOLEAN-MINIMIZATION-SPEC.md`                                                           |   ✅ |    ✅ | Implemented on `mengwong/symbeval` (see commits below).                                                                                                    |
|   2 | TS partial-eval analyzer for ladder UI          | `doc/dev/specs/todo/PARTIAL-EVAL-VISUALIZER-SPEC.md`                                                        |   ✅ |    ✅ | `ts-shared/l4-ladder-visualizer/src/lib/eval/partial-eval.ts` + tests.                                                                                     |
|   3 | Decision-service `/query-plan` endpoint         | `doc/dev/specs/todo/BOOLEAN-MINIMIZATION-SPEC.md`                                                           |   ✅ |    ✅ | Includes per-ask schema via `QueryAsk.schema` for elicitation (`jl4-decision-service/src/Backend/DecisionQueryPlan.hs`).                                   |
|   4 | Stable atom IDs (`atomId`)                      | `doc/dev/specs/todo/BOOLEAN-MINIMIZATION-SPEC.md`                                                           |   ✅ |    ✅ | Enables stable frontend caching and cross-process references.                                                                                              |
...
|   8 | Provenance beyond top-level params              | `doc/dev/specs/todo/BOOLEAN-MINIMIZATION-SPEC.md`                                                           |   ✅ |    ✅ | `/query-plan` now includes `asks[*].path :: [Text]` (segments) alongside `key`; schema lookup tolerates dotted segments.                                   |
...
|  10 | “TYPICALLY” priors in optimizer                 | `doc/dev/specs/todo/TYPICALLY-DEFAULTS-SPEC.md` / `doc/dev/specs/todo/TYPICALLY-STATUS-AND-NEXT-STEPS.html` |   ⚠️ |    ⚠️ | Deferred; would change relevance/prioritization semantics.                                                                                                 |
