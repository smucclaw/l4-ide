...
**~~Parameterized types:~~** **Fixed.** The DECLARE's GIVEN type parameters are now propagated to the synthetic DECIDE's type signature. For `GIVEN a IS A TYPE DECLARE Box a HAS f IS A BOOLEAN MEANS ...`, the synthetic function is generated as `GIVEN a IS A TYPE GIVEN _self IS A (Box a) GIVETH A BOOLEAN`.

**~~Equality operator:~~** **Fixed.** The LET binding approach for sibling fields was replaced with direct expression rewriting (`rewriteFieldRefs` in `Desugar.hs`). Bare field references in the MEANS expression are now rewritten to `_self's field` projections, eliminating the duplicate name bindings that caused ambiguity with polymorphic operators like `=`. The rewrite respects shadowing: WHERE, LET, and Lam bindings remove their bound names from the rewrite set.
