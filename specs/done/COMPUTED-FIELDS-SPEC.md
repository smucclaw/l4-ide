...
**Parameterized types:** `GIVEN a IS A TYPE DECLARE Box a HAS ... MEANS ...` fails because the synthetic DECIDE generated by desugaring does not propagate the type parameter `a`. The selector function is generated without the `GIVEN a IS A TYPE` prefix, so `a` is out of scope.

**Equality operator:** Using `=` in a computed field's MEANS expression (e.g., `MEANS \`x\` = \`y\``) triggers "multiple definitions" ambiguity. The overloaded `=` operator interacts with the dual name bindings (selector + LET binding) created by desugaring. Arithmetic operators (`+`, `-`, `*`, `>=`) work fine. This may be related to how the type-directed name resolution handles overloaded operators when there are duplicate bindings with identical types and source ranges.
