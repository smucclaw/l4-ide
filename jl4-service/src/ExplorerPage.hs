...
renderExplorerPageBS registry = LBS.fromStrict $ Text.Encoding.encodeUtf8 $ Text.unlines
  ...
  , "            <code>auto</code> (default) <code>rules</code> if &le; 10, otherwise <code>rule-tools</code><br>"
  ...
  , "    // When the explorer is served via the consolidated proxy on a"
  , "    // *.legalese.cloud subdomain, the canonical MCP address lives on the"
  , "    // shared mcp.legalese.cloud host with the org slug in the path. Local"
  , "    // dev and other deployments keep the same-origin /.mcp address."
  , "    var hostname = window.location.hostname;"
  , "    var MCP_URL;"
  , "    if (hostname.endsWith('.legalese.cloud') && !hostname.startsWith('mcp.')) {"
  , "      var orgSlug = hostname.split('.')[0];"
  , "      MCP_URL = 'https://mcp.legalese.cloud/' + orgSlug;"
  , "    } else {"
  ...
  , "    document.getElementById('mcp-config').textContent = JSON.stringify({ mcpServers: { 'l4-rules': { type: 'http', url: MCP_URL, headers: { Authorization: 'Bearer sk_...' } } } }, null, 2);"
  ...
  , "          }"
  , "        })"
  , "        .catch(function(){btn.disabled=false;btn.textContent='Retry';var m3=sec.querySelector('.deploy-meta');if(m3) m3.textContent='request failed';});"
  , "    }"
  , ""
