...
renderOrgWebMCPScript = RawJs $ LBS.fromStrict $ Text.Encoding.encodeUtf8 $ Text.unlines
  ...
  , "  }"
  , ""
  , "  function matchesScope(deploymentId, functionName) {"
  , "    if (!scope) return true;"
  , "    return scope.split(',').some(function(pattern) {"
  , "      var parts = pattern.trim().split('/');"
  , "      var depPat = parts[0], fnPat = parts[1];"
  , "      if (depPat !== '*' && depPat !== deploymentId) return false;"
  , "      if (!fnPat || fnPat === '*') return true;"
  , "      return fnPat === functionName;"
  , "    });"
  , "  }"
  , ""
  ...
  , "  }"
  , ""
  , "  // Fetch all deployments, then metadata for each ready one"
  , "  doFetch(baseUrl + '/deployments')"
  , "    .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })"
  , "    .then(function(deployments) {"
  , "      var readyDeps = deployments.filter(function(d) { return d.dsStatus === 'ready'; });"
  , "      // Check scope: skip deployments not in scope at all"
  , "      if (scope) {"
  , "        readyDeps = readyDeps.filter(function(d) {"
  , "          return scope.split(',').some(function(p) {"
  , "            var depPat = p.trim().split('/')[0];"
  , "            return depPat === '*' || depPat === d.dsId;"
  , "          });"
  , "        });"
  , "      }"
  , ""
  , "      // Fetch metadata for each ready deployment"
  , "      return Promise.all(readyDeps.map(function(dep) {"
  , "        return doFetch(baseUrl + '/deployments/' + dep.dsId + '/openapi.json')"
  , "          .then(function(r) { return r.ok ? r.json() : null; })"
  , "          .then(function(meta) { return meta ? { id: dep.dsId, status: dep.dsStatus, meta: meta } : null; })"
  , "          .catch(function() { return null; });"
  , "      })).then(function(results) {"
  , "        return { all: deployments, loaded: results.filter(Boolean) };"
  , "      });"
  , "    })"
  , "    .then(function(data) {"
  , "      var allFns = [];"
  , "      var deploymentInfos = data.all.map(function(d) { return { id: d.dsId, status: d.dsStatus }; });"
  , ""
  , "      data.loaded.forEach(function(dep) {"
  , "        (dep.meta.metaFunctions || []).forEach(function(fn) {"
  , "          if (!matchesScope(dep.id, fn.fsName)) return;"
  , "          allFns.push({"
  , "            deployment: dep.id,"
  , "            name: fn.fsName,"
  ...
  , "          return fn.name.toLowerCase().indexOf(q) >= 0 || fn.description.toLowerCase().indexOf(q) >= 0 || fn.deployment.toLowerCase().indexOf(q) >= 0 || fn.parameterNames.some(function(p) { return p.toLowerCase().indexOf(q) >= 0; });"
  , "        });"
