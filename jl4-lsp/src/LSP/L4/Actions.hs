...
visualise mtcRes (getRecVis, setRecVis) verTextDocId msrcPos = do
  ...
  let recentlyVisualisedDecide (MkDecide Anno {range = Just range, extra = Extension {resolvedInfo = Just (MyKnownTerm ty _)}} _tydec appform _expr) vizState
  ...
  where
    x =
      case i of
        ...
        MyKnownType k       -> mdCodeBlock $ prettyLayout $ typeFunction k
        MyKnownTypeVariable -> "TYPE VAR"
