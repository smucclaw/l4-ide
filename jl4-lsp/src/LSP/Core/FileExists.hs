...
getFileExists = use_ GetFileExists
...
fileExistsRules :: Recorder (WithPriority Log) -> Maybe LspSink -> Rules ()
fileExistsRules recorder lspSink = do
  supportsWatchedFiles <- case lspSink of
    Nothing      -> pure False
    Just sink -> do
      clientCaps <- liftIO $ currentClientCapabilities sink
      pure $ isWatchSupported clientCaps
  ...
  let globs = watchedGlobs opts
      ...
      isWatched = if supportsWatchedFiles
        then \f -> do
            case uriToNormalizedFilePath f of
              Just nfp ->  do
...
getFileExistsVFS uri = do
  ...
  case (vf, uriToNormalizedFilePath uri)of
    (Just _, _) -> pure True
    (_, Just file) -> do
      liftIO $ handle (\(_ :: IOException) -> return False) $
         Dir.doesFileExist (fromNormalizedFilePath file)
    (Nothing, Nothing) -> pure False -- FIXME: no bueno
