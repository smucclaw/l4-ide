...
--
-- Copied from haskell/haskell-language-server ghcide/src/Development/IDE/Core/PositionMapping.hs
-- Commit: b87bdb9ebd44b4494e3e9e9c304d4dbf8810905f
{-# LANGUAGE ExplicitNamespaces #-}
{-# LANGUAGE ViewPatterns #-}
{-# LANGUAGE FieldSelectors #-}
module LSP.Core.PositionMapping
  ( PositionMapping(..)
  , PositionResult(..)
  , lowerRange
  , upperRange
...
applyChange p (TextDocumentContentChangeEvent (InL x)) = PositionDelta
    { toDelta = toCurrent (x ^. L.range) (x ^. L.text) <=< p.toDelta
    , fromDelta = p.fromDelta <=< fromCurrent (x ^. L.range) (x ^. L.text)
    }
