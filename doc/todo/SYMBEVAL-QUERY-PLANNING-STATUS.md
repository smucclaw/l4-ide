...
1. Extend provenance beyond top-level params (multiple `InputRef`s, richer `asks[*].path` semantics, better aggregation for record/array containers).
2. Improve schema summaries (e.g. `Record{...}`, `List<T>`, enum previews) and consider tooltips for multiple asks per atom.
