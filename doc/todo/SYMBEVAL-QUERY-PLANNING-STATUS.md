...
1. Update remaining consumers (jl4-web + any API clients) to use `asks[*].path` + `asks[*].schema` (prefer `@repo/decision-service-types`) instead of parsing `key`/`label` strings.
