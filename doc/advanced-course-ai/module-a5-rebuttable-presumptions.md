...
### Runtime wrappers vs. mixfix syntax

Many examples in this module keep the legal prose feel by using mixfix function names. Once TYPICALLY defaults are involved, the compiler additionally generates helper functions named `'presumptive <original name>'` (plus strict variants) with plain prefix names and `Maybe`-wrapped parameters. Those prefix wrappers are what the Decision Service API **and** the CLI `#PEVAL`/`#PEVALTRACE`/`#PASSERT` directives call today; mixfix never crosses that boundary and CLI output now shows `JUST …` / `NOTHING` so you can see which defaults fired. When documenting endpoints or sharing JSON payloads, publish the canonical prefix names (e.g. `'presumptive may purchase alcohol'`) so downstream systems can pass them verbatim.

> **Transitive behavior (roadmap):** Wrapper bodies currently call the original functions. The inner-call rewrite will be reinstated after the wrapper invocation contract (automatic `NOTHING` injection and `JUST` wrapping) is finalized. Until then, call the downstream wrappers explicitly whenever a decision depends on another defaulted function.
