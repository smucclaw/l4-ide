...
validateFunction fn = do
 ...
 where
  validateImplementation = undefined
...
batchRequestEncoder br =
  Aeson.object
    [ "outcomes" .= fmap outcomesEncoder (br.batchRequestOutcomes)
    ]
...
outcomesEncoder (OutcomePropertyObject o) =
  Aeson.object
    [ "@id" .= o.outcomeObjectId
    , "showSilent" .= o.outcomeObjectShowSilent
    , "showInvisible" .= o.outcomeObjectShowInvisible
    , "resolveIndecisionRelationships" .= o.outcomeObjectResolveIndecisionRelationships
    , "knownOutcomeStyle" .= fmap outcomeStyleEncoder (o.outcomeObjectKnownOutcomeStyle)
    , "unknownOutcomeStyle" .= fmap outcomeStyleEncoder (o.outcomeObjectUnknownOutcomeStyle)
    ]
...
batchResponseEncoder br =
  Aeson.object $
    [ "cases" .= fmap outputCaseEncoder br.batchResponseCases
    , "summary" .= outputSummaryEncoder br.batchResponseSummary
    ]

outputSummaryEncoder :: OutputSummary -> Aeson.Value
outputSummaryEncoder os =
  Aeson.object
    [ "casesRead" .= os.outputSummaryCasesRead
    , "casesProcessed" .= os.outputSummaryCasesProcessed
    , "casesIgnored" .= os.outputSummaryCasesIgnored
    , "processorDurationSec" .= toSci os.outputSummaryProcessorDurationSec
    , "processorCasesPerSec" .= toSci os.outputSummaryCasesPerSec
    , "processorQueuedSec" .= toSci os.outputSummaryProcessorQueuedSec
    ]
 where
...
outputCaseEncoder oc =
  Aeson.object $
    [ "@id" .= oc.outputCaseId
      ...
      <> [ (Aeson.fromText k .= v)
         | (k, v) <- Map.assocs $ oc.outputCaseAttributes
         ]
