...
generateNodeRunner wasmPath fe resolved =
  ...
  in unlines
    ...
    , "//   days-since-1969-12-28 (the last Sunday before the Unix"
    , "//   epoch), so 'date % 7' gives the day-of-week with Sunday=0,"
    , "//   matching the library's weekday constants."
    , "// TIME: f64 seconds since midnight"
    , "// DATETIME: pointer (bit-cast to f64) to a 2-slot record [utcSeconds, tzNamePtr]"
    , "const DATE_EPOCH_SHIFT = 4;  // days from 1969-12-28 (Sun) to 1970-01-01 (Thu)"
    , "function dateFromDMY(d, m, y) { return Math.floor(Date.UTC(y, m - 1, d) / 86400000) + DATE_EPOCH_SHIFT; }"
    , "function dateToUnixDays(x) { return Number(x) - DATE_EPOCH_SHIFT; }"
    ...
    , "}"
    , "function timeFromHMS(h, m, s) { return h * 3600 + m * 60 + s; }"
    ...
    , "  const t = Number(sec);"
    , "  const h = Math.floor(t / 3600), mi = Math.floor((t % 3600) / 60), s = Math.floor(t % 60);"
    ...
    , "       + ' UTC';"
    , "}"
    , "// JUST-wrapped result helper used by __l4_to_date / __l4_to_time / __l4_to_datetime."
    ...
    , "  // DATE (format=date) and TIME (format=time) wire as strings but"
    , "  // the compiled code expects a numeric f64 (days / seconds)."
    ...
    , "    return idx >= 0 ? idx : 0;"
    , "  }"
    , "  // DATE / TIME / DATETIME: wire format is a string; internal is numeric"
    ...
    , "    __l4_free:  (_p) => {},"
    , "    // libm functions llc emits as calls when lowering MLIR arith"
    , "    // operations that don't have a native wasm32 instruction."
    , "    fmod:  (a, b) => Number(a) % Number(b),"
    , "    fmodf: (a, b) => Number(a) % Number(b),"
    , "    remainder:  (a, b) => Number(a) - Math.round(Number(a) / Number(b)) * Number(b),"
    , "    remainderf: (a, b) => Number(a) - Math.round(Number(a) / Number(b)) * Number(b),"
    , "    trunc: Math.trunc,"
    , "    // DATE primitives (f64 days since Unix epoch)"
    , "    __l4_date_from_dmy:    (d, m, y) => dateFromDMY(Number(d), Number(m), Number(y)),"
    , "    // Our internal DATE rep is already Sunday-aligned, so"
    , "    // DATE_SERIAL / DATE_FROM_SERIAL are identity."
    , "    __l4_date_from_serial: (n) => Number(n),"
    , "    __l4_date_serial:      (d) => Number(d),"
    , "    __l4_date_day:         (d) => new Date(dateToUnixDays(d) * 86400000).getUTCDate(),"
    , "    __l4_date_month:       (d) => new Date(dateToUnixDays(d) * 86400000).getUTCMonth() + 1,"
    , "    __l4_date_year:        (d) => new Date(dateToUnixDays(d) * 86400000).getUTCFullYear(),"
    , "    __l4_datevalue:        (sPtrF) => {"
    , "      const s = readCString(Number(f64ToU64(sPtrF)));"
    , "      const v = parseDateStr(s);"
    , "      if (!Number.isFinite(v)) { return mbNothing(); }"
    , "      return mbJustF64(v);"
    , "    },"
    , "    __l4_to_date:          (sPtrF) => {"
    , "      const s = readCString(Number(f64ToU64(sPtrF)));"
    ...
    , "      return mbJustF64(v);"
    , "    },"
    , "    // TIME primitives (f64 seconds since midnight)"
    , "    __l4_time_from_hms:    (h, m, s) => timeFromHMS(Number(h), Number(m), Number(s)),"
    ...
    , "    __l4_to_time:          (sPtrF) => {"
    , "      const s = readCString(Number(f64ToU64(sPtrF)));"
    , "      const v = parseTimeStr(s);"
    , "      if (!Number.isFinite(v)) { return mbNothing(); }"
    , "      return mbJustF64(v);"
    , "    },"
    , "    // DATETIME primitives (pointer to [utcSeconds, tzName])"
    , "    __l4_datetime_from_dtz: (d, t, tz) => datetimeFromDTZ(d, t, tz),"
    , "    __l4_datetime_date:     (dtF) => {"
    , "      const p = Number(f64ToU64(dtF));"
    , "      if (!p) return 0;"
    , "      return Math.floor(memView.getFloat64(p, true) / 86400) + DATE_EPOCH_SHIFT;"
    , "    },"
    ...
    , "      const tzPtr = Number(memView.getBigUint64(p + 8, true));"
    , "      return u64ToF64(tzPtr);"
    , "    },"
    ...
    , "    __l4_acos:  Math.acos,"
    , "    __l4_atan:  Math.atan,"
    , "    __l4_trunc: (x, d) => { const m = Math.pow(10, Number(d)); return Math.trunc(Number(x) * m) / m; },"
    ...
    , "    __l4_to_upper:      (sF) => u64ToF64(writeString(readCString(Number(f64ToU64(sF))).toUpperCase())),"
    , "    __l4_to_lower:      (sF) => u64ToF64(writeString(readCString(Number(f64ToU64(sF))).toLowerCase())),"
    , "    __l4_trim:          (sF) => u64ToF64(writeString(readCString(Number(f64ToU64(sF))).trim())),"
    , "    __l4_contains:      (hF, nF) => readCString(Number(f64ToU64(hF))).includes(readCString(Number(f64ToU64(nF)))) ? 1 : 0,"
    , "    __l4_starts_with:   (hF, nF) => readCString(Number(f64ToU64(hF))).startsWith(readCString(Number(f64ToU64(nF)))) ? 1 : 0,"
    , "    __l4_ends_with:     (hF, nF) => readCString(Number(f64ToU64(hF))).endsWith(readCString(Number(f64ToU64(nF)))) ? 1 : 0,"
    , "    __l4_index_of:      (hF, nF) => readCString(Number(f64ToU64(hF))).indexOf(readCString(Number(f64ToU64(nF)))),"
    , "    __l4_char_at:       (sF, iF) => u64ToF64(writeString(readCString(Number(f64ToU64(sF))).charAt(Number(iF)))),"
    , "    __l4_substring:     (sF, iF, jF) => u64ToF64(writeString(readCString(Number(f64ToU64(sF))).substring(Number(iF), Number(jF)))),"
    ...
    , "    },"
    , "    // Nullary temporal. TODAY and NOW return Unix serials; CURRENTTIME"
    , "    // returns a TIME (seconds-since-midnight UTC)."
    , "    __l4_today_serial: () => Math.floor(Date.now() / 86400000) + DATE_EPOCH_SHIFT,"
    , "    __l4_now_serial:   () => Date.now() / 1000,"
    , "    __l4_current_time: () => { const d = new Date(); return d.getUTCHours()*3600 + d.getUTCMinutes()*60 + d.getUTCSeconds(); },"
    , "    __l4_timezone_name: () => u64ToF64(writeString('UTC')),"
    , "    // JSON."
    , "    __l4_json_encode: (vF) => u64ToF64(writeString(String(Number(vF)))),"
    , "    __l4_json_decode: (sF) => {"
    , "      try { const v = JSON.parse(readCString(Number(f64ToU64(sF)))); return typeof v === 'number' ? mbJustF64(v) : mbNothing(); }"
    , "      catch { return mbNothing(); }"
    , "    },"
    , "  }"
    , "};"
    , ""
    , "// Unsupported higher-order / unresolved library names appear as"
    , "// MLIR externs; Node's WebAssembly.instantiate requires every"
    , "// declared import to be present. Wrap env in a Proxy that returns"
    , "// a throwing stub for anything we didn't implement — modules that"
    , "// never actually call those symbols instantiate fine."
    , "const envProxy = new Proxy(imports.env, {"
    , "  get(target, prop) {"
    , "    if (prop in target) return target[prop];"
    , "    return () => { throw new Error('l4: unresolved runtime symbol \"' + String(prop) + '\" (higher-order / unsupported library feature)'); };"
    , "  }"
    , "});"
    , ""
    , "WebAssembly.instantiate(buf, { env: envProxy }).then(({instance}) => {"
