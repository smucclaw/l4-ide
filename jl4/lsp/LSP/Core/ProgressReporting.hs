...
module LSP.Core.ProgressReporting
  ( ProgressEvent (..),
...
progressReportingNoTrace todo done (Just notifier) title optProgressStyle = do
  progressState <- newVar NotStarted
  let _progressUpdate event = liftIO $ updateStateVar $ Event event
      _progressStop = updateStateVar StopProgress
      updateStateVar = modifyVar_ progressState . updateState (progressCounter notifier title optProgressStyle todo done)
...
progressReporting (Just notifier) title optProgressStyle = do
  inProgressState <- newInProgress
  progressReportingInner <- progressReportingNoTrace (readTVar $ todoVar inProgressState)
                                (readTVar $ doneVar inProgressState) (Just notifier) title optProgressStyle
  let
...
progressCounter ::
  ...
  STM Int ->
  IO ()
progressCounter sink title optProgressStyle getTodo getDone =
  withClientProgress sink title NotCancellable $ \update -> loop update 0
  where
    loop :: forall a b . (ProgressAmount -> IO a)-> UInt -> IO b
    ...
    loop update prevPct = do
      ...
      _ <- update (ProgressAmount (Just nextPct) (Just $ T.pack (show done) <> "/" <> T.pack (show todo)))
      loop update nextPct
