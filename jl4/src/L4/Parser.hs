...
topdecl = attachAnno $
      Declare   emptyAnno <$> annoHole declare
  <|> Decide    emptyAnno <$> annoHole decide
  <|> Assume    emptyAnno <$> annoHole assume
...
infix2 f op l r =
  f (fixAnnoSrcRange $ mkHoleAnno (rangeOf l) <> mkSimpleEpaAnno (lexToEpa op) <> mkHoleAnno (rangeOf r)) l r

infix2' :: HasSrcRange (a n) => (Anno -> a n -> a n -> a n) -> Anno -> a n -> a n -> a n
infix2' f op l r =
  f (fixAnnoSrcRange $ mkHoleAnno (rangeOf l) <> op <> mkHoleAnno (rangeOf r)) l r
...
projection =
      ...
      (\ n ns ->
        foldl'
          (\e (gen, n') ->
            Proj (fixAnnoSrcRange $ mkHoleAnno (rangeOf e) <> mkSimpleEpaAnno (lexToEpa gen) <> mkHoleAnno (rangeOf n'))
              e
              n'
          )
          (Var (fixAnnoSrcRange $ mkHoleAnno (rangeOf n)) n)
          ns
      )
  <$> name
...
annoHole p = Compose $ fmap (\ a -> WithAnno (mkHoleAnno $ rangeOf a) a) p
...
inlineAnnoHole p = (\ (WithAnno ann e) -> setAnno (fixAnnoSrcRange $ mkAnno (inlineAnnos ann.payload (getAnno e).payload)) e) <$> getCompose p
  where
    inlineAnnos []                 _   = []
    inlineAnnos (AnnoHole _ : as1) as2 = as2 ++ as1
    inlineAnnos (a : as1)          as2 = a : inlineAnnos as1 as2
...
mkHoleAnno :: Maybe SrcRange -> Anno_ t e
mkHoleAnno mRange =
  mkAnno [mkHole' mRange]
