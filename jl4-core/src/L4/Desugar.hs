...
--   [["(", _, " PLUS "], [_, ")"]]
-- @
rewriteBinOpAnno :: Expr Name -> NonEmpty (Expr Name) -> Desugar [Expr Name]
rewriteBinOpAnno expr allExprArgs = go annoPieces [] allExprArgs
  where
  annoPieces = (getAnno expr).payload

  go :: [AnnoElement] -> [AnnoElement] -> NonEmpty (Expr Name) -> Desugar [Expr Name]
  go pieces leadingCsn (arg :| args) =
    case pieces of
      (AnnoHole{} : rest) ->
        let
          (csns, holeAndCsn) = span (isJust . preview #_AnnoCsn) rest
          rewrittenArg = setAnno (surroundWithCsn (getAnno arg) (reverse leadingCsn) csns) arg
        in
          case holeAndCsn of
            [] -> case args of
              [] -> pure [rewrittenArg]
              (_ : _) -> internalError $ InternalAnnoRewritingError expr (HoleInfo (length allExprArgs) numberOfAnnoHoles)
            _ -> case args of
              [] -> internalError $ InternalAnnoRewritingError expr (HoleInfo (length allExprArgs) numberOfAnnoHoles)
              (x : xs) -> do
                rewrittenArgs <- go holeAndCsn [] (x :| xs)
                pure (rewrittenArg : rewrittenArgs)
      (csn@AnnoCsn{} : rest) ->
        -- Accumulate the leading concrete syntax nodes.
        go rest (csn:leadingCsn) (arg :| args)
      [] ->
        -- No concrete syntax nodes  to attach any more, so just return the rest.
        -- Strictly speaking, this is incorrect, there is an implicit assumption
        -- that the number of 'Hole's matches the number of arguments, otherwise
        -- the incoming 'Anno' is wrong.
        pure (arg:args)

  numberOfAnnoHoles = length $ filter (isJust . preview #_AnnoHole) annoPieces

  surroundWithCsn :: Anno -> [AnnoElement] -> [AnnoElement] -> Anno
  surroundWithCsn a before after =
    fixAnnoSrcRange
      Anno
        { extra = a.extra
        , range = a.range
