...
2. **`matchLinearAfterHeadKeyword`** returns `Maybe [MixfixArgMatch (Expr Name)]`, preserving info about which positions are keywords vs parameters.
...
- Error messages don't distinguish keyword mismatches from other errors

## Enhancement 1: Better Error Messages for Keyword Mismatches
...
1. **Create new error types** in `L4/TypeCheck/Types.hs`:
   ```haskell
...
2. **Modify `matchLinearAfterHeadKeyword`** to return `Either MixfixMatchError [MixfixArgMatch (Expr Name)]` instead of `Maybe [MixfixArgMatch (Expr Name)]`:
   ```haskell
   matchLinearAfterHeadKeyword
     :: [MixfixPatternToken]
     -> [Expr Name]
     -> Either MixfixMatchError [MixfixArgMatch (Expr Name)]
   ```

3. **Propagate errors up** through `matchMixfixPattern`, `matchParamFirstPattern`, `tryMatchAnyPattern`, and `tryMatchMixfixCall`.

4. **In the `App` case of `inferExpr`** (around line 1180), catch these errors and convert to user-friendly `CheckError` messages.

5. **Add fuzzy matching** for keyword suggestions:
   ```haskell
   suggestKeywords :: RawName -> [RawName] -> [RawName]
   suggestKeywords unknown knowns =
     filter (\k -> editDistance unknown k <= 2) knowns
   ```

### Files to Modify

- `jl4-core/src/L4/TypeCheck/Types.hs` - Add `MixfixMatchError` type
- `jl4-core/src/L4/TypeCheck.hs` - Modify matching functions, add error handling
...
|-----------|------|---------------|
| MixfixArgMatch type | L4/TypeCheck/Types.hs | 188-196 |
| matchLinearAfterHeadKeyword | L4/TypeCheck.hs | 2236 |
| extractParamArgs | L4/TypeCheck.hs | 2261 |
| tryMatchMixfixCall | L4/TypeCheck.hs | 2071 |
| App case in inferExpr | L4/TypeCheck.hs | 1158-1206 |
| Keyword built-in | L4/TypeCheck/Environment.hs | 25, 96-98, 282-285 |
| MixfixInfo type | L4/TypeCheck/Types.hs | 198-200 |
| MixfixPatternToken | L4/TypeCheck/Types.hs | 180-186 |
...
- All 399 existing tests must continue to pass after each enhancement
